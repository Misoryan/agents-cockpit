...
- `codex_native.py` keeps the Codex session class while delegating app-server client, routing, requests, pending request state, replay facade/helpers, turn/thread lifecycle, notification adapter, session state persistence/recovery, text/form/history, thread-history conversion, and terminal interaction helpers to `codex_*.py` modules.
