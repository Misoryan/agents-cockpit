...
- `codex_native.py` keeps the Codex session class while delegating app-server client, routing, requests, pending request state, replay, text/form/history, thread-history conversion, terminal interaction, and notification lifecycle helpers to `codex_*.py` modules.
