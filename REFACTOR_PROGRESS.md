...
- MCP status visibility now has a first slice: `/mcp-status [full|tools]` and
...
- Codex slash, lifecycle, goal, and manual MCP helpers moved into
  ...
  `/goal`, `/mcp-status`, `/mcp-resources`, `/mcp-resource`, and `/mcp-tool`
  behavior behind focused adapters.
...
- Codex app-server standalone `command/exec` now has a live smoke covering
  buffered output, streamed stdout/stderr, streamed stdin, and terminate;
  `CodexAppServerClient` can route connection-scoped `command/exec/outputDelta`
...
- The Codex launch modal now includes a read-only diagnostics foldout fed by
  `/api/codex_options`, covering cwd, Codex home, state dir, workspace roots,
  inherited config, config layer count, account summary, and discovery errors.
...
- `tools/check_hardened_profile.py` now makes the recommended tunnel/shared-use
  ...
  bounded session TTL.
- `docs/state-changing-api-risk-matrix.md` and
  `tools/check_state_changing_api_risks.py` now keep browser, manager, and
...
- `codex_account.py` now backs `/account-status [basic|full]` with read-only
  `account/read`, `account/rateLimits/read`, and `account/usage/read` calls,
...
python tools\codex_command_exec_smoke.py --cwd .
```
