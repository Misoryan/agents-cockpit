...
更新时间：2026-07-17
项目：`E:\tools\codex-web`
当前基线：`main`（截至 2026-07-17 native tool-helper extraction checkpoint）
...
| 代码可维护性 | 74-76% | manager/common/native/frontend 已多轮拆分；工具卡、结果卡、pending 卡、terminalInteraction 卡、text/thinking、tool helper 和 sidebar Codex lifecycle actions 已从主事件/舞台/sidebar 模块继续拆出；`CodexSession`、sidebar list rendering、web 入口和协议路由仍是复杂热点。 |
...
- 前端 renderer 继续收口：tool-use、tool-result、tool helpers、pending cards、terminalInteraction cards、text/thinking helpers、sidebar Codex action helpers 已拆出；下一刀优先评估 sidebar list/history row renderer 或 push/notification 边界。
...
1. 继续 Phase 2 剩余结构收口：pending/form renderer、terminalInteraction card、text/thinking renderer、tool helpers、sidebar Codex actions 已完成，下一步优先拆 sidebar list/history row renderer 或 push/notification 边界中最容易回归的部分，保持行为不变。
