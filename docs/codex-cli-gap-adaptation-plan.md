...
- 已完成结构小步：terminalInteraction 状态和 `command/exec/*` 映射逻辑已抽到 `codex_terminal.py`，pending approval/ask/form 状态和 snapshot/replay helper 已抽到 `codex_pending.py`；`CodexSession` 保留兼容包装。
...
1. 先做真实浏览器/手机 visual smoke 清单或自动化，因为这是当前“多端流畅体验”剩余证据缺口。清单和记录模板已落到 `docs/codex-visual-smoke-checklist.md` 与 `tools/codex_visual_smoke_report.py`；headless 双浏览器自动化已落到 `tools/codex_browser_smoke.py`，下一步是实际跑一次手机/窄屏 V01-V10 记录。
